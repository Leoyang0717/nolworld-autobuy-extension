...
| 功能 | 適用頁面 | 說明 |
|------|----------|------|
| 🚀 區域輪詢刷票 | gpoticket（葡萄座位圖） | 輪流點擊目標區域，找到綠葡萄自動選位、選票數、進到個資頁 |
...
| 🔒 驗證碼偵測 | gpoticket | 刷票中出現驗證碼自動停止並警示，完成驗證後再手動重啟 |
...
**🔒 驗證碼偵測**：刷票過程中若跳出驗證碼（hCaptcha / 滑塊），程式會**自動停止**並播放警示音 + 桌面通知。請手動完成驗證後，再重新點擊「開始搶票」。
...
> 💡 頻繁遇到 Access Denied 代表輪詢速率踩到網站的防護線，建議把換區間隔調高（例如 1500~3000ms）。
