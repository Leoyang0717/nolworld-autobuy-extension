...
| 🛰️ 背景剩餘座位監控（新）| gpoticket | 與盲輪／偵察**並行**：每隔數秒打一次全區剩餘座位 API 掃全場，發現有票的區就自動指揮主模式改刷該區，售完或逾時無斬獲再切回指定區。專治「有顯示剩餘數」的活動臨時釋票 |
| 🚫 排除區域（新）| gpoticket | 絕對黑名單，盲輪／偵察／監控**全部跳過**，優先權高於指定區。放掉不想要的座位重搶時，把該區加進來就不會又抓回同一區 |
...
在「目標區域」下方的「**排除區域**」欄位，可輸入絕對不想要的區（操作方式與目標區相同）。排除清單**優先權高於指定區**，盲輪、偵察、背景監控三邊都會跳過。

- 若某區同時出現在「指定」與「排除」兩份清單，欄位下方會即時跳出警訊，並以**排除為準**（不會刷）。
- 主要用途見下方[背景剩餘座位監控](#背景剩餘座位監控動態切換到有票區)的「放票重搶」流程。
...
部分活動的座位圖會顯示各區「剩餘席數」。這類活動的痛點是：**不主動查詢就不會更新**，你盯著指定的那幾區，可能錯過別區突然冒出的 1 個空位。背景監控就是為此設計——它**與盲輪／偵察並行**，在背後每隔數秒打一次「全區剩餘座位 API」（一個請求即掃全場），發現有票區就指揮主模式改刷該區。

> ✅ 只對「**有顯示剩餘數**」的活動有效。沒有剩餘數的活動開了也無妨，只是掃不到有效資料。

### 開啟與設定

在「**🛰️ 背景剩餘座位監控**」區塊勾選「啟用」，會展開兩個設定：

| 設定 | 說明 | 建議值 |
|------|------|--------|
| 掃描間隔（秒）| 每隔幾秒打一次全區 API | 5 秒（別調太小，是額外請求）|
| 剩餘門檻（≥）| 剩餘席數達多少才算「有票」並切過去 | 1 |

啟動後 log 會出現 `🛰️ 背景剩餘監控啟動…`，以及一行遮罩過 SessionId 的 `🛰️ 監控 API 就緒：…` 供你核對。

### 運作規則

- **主模式照跑、時限照算**：監控只負責「看與指揮」，盲輪／偵察繼續刷、持續時間不會停。
- **切區防橫跳**：鎖定一區後**至少停留 60 秒**集中刷，避免在多區之間反覆橫跳、每區都搶不到。
- **優先序**：多區同時有票時，先挑你「指定清單」內有票的區（依清單順序），都沒有才挑非指定區（依 API 順序，通常越靠舞台越前）。
- **換區／切回**：鎖定區滿 60 秒仍沒搶到、且他區有票 → 跳去他區；該區售完且無其他有票區 → 切回你原本的指定區輪刷。
- **全票種都搶**：VIP／指定席／站立不過濾（不想要的區請用下方「排除區域」擋掉）。
- **偵察模式心跳**：偵察鎖定某區時逐次請求本來不印，程式會每 5 秒補一行 `🎯 XXX 集中刷第 N 次·剩餘 X·仍未搶到`，讓你確定它還在打、剩餘還在變。

log 範例：

```
🛰️ 全區掃描：1 區有票（202×3）
🎯 切換到有票區 202（剩餘 3）→ 主模式改刷此區
🎯 202 集中刷第 5 次·剩餘 3·仍未搶到
🎯 偵察命中：202 區有 1 個可選座位，開衝！
↩️ 202 售完且無其他有票區，回到指定區域輪刷
```

### 搭配「排除區域」：放掉不想要的座位再重搶

搶到的位子不喜歡、想放掉重搶時，會遇到一個陷阱：**你一放票，那一區的剩餘數立刻跳回 >0，背景監控反而會優先鎖回你剛嫌棄的那一區。** 用排除區域解決：

1. 在網站上放掉不要的座位
2. 把該區代碼加進「**排除區域**」
3. 重新按「**🚀 開始搶票**」

排除區在盲輪、偵察、監控三邊一律跳過，就不會又抓回同一區。

> ⚠️ **整區排除是「連坐」**：排除某區＝該區所有座位都不要（含好位子）。目前不支援「同區只跳過某個座位」。
>
> ⚠️ **排除清單在「按下開始」的當下讀取**，跑到一半改不會即時生效，要停下重按開始。

### 注意事項

- **RemainCnt 可能有延遲**：顯示剩餘 3、切過去卻撲空，是伺服器數字快取的特性，非程式錯誤——監控會繼續刷到真的命中或該區歸零。
- **監控是額外請求**：掃描間隔太小會增加觸發風控（Access Denied／驗證）的機會。若被擋變頻繁，把掃描間隔調大。被擋或開衝期間，監控會自動跳過該次掃描，不會火上加油。
